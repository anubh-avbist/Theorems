...
\subsection{Definitions}
    ...
    \begin{definition}[Connected Space]
        A topological space is called connected if it cannot be expressed as the union of two disjoint, non-empty open subsets.
    \end{definition}
...
\begin{theorem}
    ...
    \begin{itemize}
        \item $X$ is connected
        \item Every continuous map from $X$ to the discrete two-point space is constant.
    \end{itemize}
\end{theorem}
...
\section{Analysis}
    ...
    \begin{definition}[Metric Space]
        A \textit{metric space} is a set $X$ equipped with a function $d:X\times X\rightarrow \mathbb{R}$ satisfying the following for all $x,y,z\in X$.
        \begin{itemize}
            \item $d(x,y)\geq 0$
        ...
        \end{itemize}
    \end{definition}
    \begin{definition}[Convergence of Real Sequences]
        A sequence $(x_n)$ on a metric space $(X,d)$ is said to \textit{converge} to a point $x\in X$, if for all $\epsilon >0$ there is some natural number $N\in\mathbb{N}$ so that whenever $n> N$, we have $d(x_n,x) < \epsilon$.
    \end{definition}
    ...
    \end{theorem}
    \begin{theorem}[Cauchy Criterion for Convergence of Real Sequences]
        Let $(x_n)$ be a sequence on a metric space $(X,d)$. The following are equivalent:
        \begin{itemize}
            \item $(x_n)$ converges
            \item For all $\epsilon> 0$ there is some $N\in\mathbb{N}$ so that whenever $m,n>N$, we have $d(x_n,x_m)<\epsilon$.
        \end{itemize}
    \end{theorem}
...
\subsubsection{Definitions}
    \begin{definition}[Ring (with Unity)]
        A \textit{ring} $R$ is a set equipped with two binary operations, $+$ and $\times$, so that the following are satisfied:
        \begin{itemize}
            \item $(R,+)$ is an abelian group
            \item $\times$ is associative: $(a\times b)\times c = a\times(b\times c)$
            \item Multiplication distributes over addition: $a\times(b+c)=a\times b + a\times c$, and $(a+b)\times c = a\times c + b\times c$ for all $a,b,c\in R$
            \item $\exists 1\in R$ so that $1\times a = a\times 1 = a$ for all $a\in R$.
        \end{itemize} 
        Note that I will always assume rings to be unital in this document unless stated otherwise.
    \end{definition}
    \begin{definition}[Subring]
        A subring of $R$ is a subgroup that is closed under multiplication.
    \end{definition}
    \begin{definition}[Commutative Ring]
        A ring $R$ is called \textit{commutative} if $a\times b = b\times a$ for all $a,b\in R$.
    \end{definition}
    \begin{definition}[Zero Divisor]
        A non-zero element $a\in R$ is called a \textit{zero divisor} if there is some nonzero element $b\in R$ so that $ab=0$.
    \end{definition}
    \begin{definition}[Unit in a Ring]
        An element of a ring $r\in R$ is called a \textit{unit} if $\exists r^{-1}\in R$ so that $rr^{-1}=r^{-1}r=1$. The set of units in $R$ is denoted $R^\times$.
    \end{definition}
    \begin{definition}[Division Ring]
        A ring $R$ is called a \textit{division ring} if $\forall a\in R\ \exists b\in R$ so that $ab=ba=1$.
    \end{definition}
    \begin{definition}[Integral Domain]
        A commutative ring $R$ is called an \textit{integral domain} if there are no zero divisors, i.e. $ab=0\implies a=0$ or $b=0$.
    \end{definition}
    \begin{definition}[Ideal]
        Let $R$ be a ring. A subset $I\subset R$ is called a \textit{left ideal} if $ra\in I$ for every $r\in R, a\in I$, and a \textit{right ideal} if $ar\in I$ for every $r\in R, a\in I$. A subset that is both a left and right ideal is simply called an \textit{ideal}.
    \end{definition}
